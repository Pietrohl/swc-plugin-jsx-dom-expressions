    ...
    C: Comments,
{
    pub fn transform_element_dom(
        ...
        transform_attributes(self, &mut node, &mut results);
            ...
            let to_be_closed = !info.last_element
                || (info.to_be_closed.is_some()
                    && (!self.config.omit_nested_closing_tags
                        || info.to_be_closed.clone().unwrap().contains(&tag_name)));
            if to_be_closed {
                let mut v = info
                    .to_be_closed
                    .clone()
    ...
    pub fn set_attr_dom(
        ...
        name = ALIASES
            .get(name.as_str())
            .map_or(name.clone(), |v| v.to_string());
        if !options.is_svg {
...
fn detect_resolvable_event_handler<C: Comments>(
    visitor: &mut TransformVisitor<C>,
    handler: &Expr,
) -> bool {
    if let Expr::Ident(id) = handler {
        if let Some(init) = visitor
            .binding_collector
            .const_var_bindings
            .get(&id.to_id())
        {
            if let Some(init) = init {
                return detect_resolvable_event_handler(visitor, &init.clone());
            } else {
                return false;
            }
        } else {
...
fn transform_attributes<C: Comments>(
    ...
    if attributes.iter().any(|attribute| match attribute {
        ...
        (attributes, spread_expr) = process_spreads(
            visitor,
            attributes,
            ProcessSpreadsInfo {
                elem: elem.clone(),
                is_svg,
                has_children,
                wrap_conditionals: visitor.config.wrap_conditionals,
            },
        );
    }
            ...
            if let PropOrSpread::Prop(p) = prop {
                return match **p {
                    ...
                    Prop::KeyValue(ref kv) => match kv.key {
                        ...
                        PropName::Str(ref s) => {
                            handle(quote_ident!(s.value.to_string()), *kv.value.clone())
                        }
    ...
    if let Some((class_list_idx, mut props, span)) = class_list_props {
        ...
        props.retain(|prop| {
            let mut handle = |name: Ident, value: Expr| {
                i += 1;
                match visitor.evaluator.as_mut().unwrap().eval(&value) {
    ...
    let class_attributes: Vec<_> = attributes
        .iter()
        .enumerate()
        .filter(|(_, a)| {
    ...
    if class_attributes.len() > 1 {
        ...
        let value = if !values.is_empty() {
            ...
            JSXAttrValue::Lit(Lit::Str(quasis[0].clone().raw.into()))
        };
        if let JSXAttrOrSpread::JSXAttr(JSXAttr { ref name, .. }) = first.1 {
            attributes[first.0] = JSXAttrOrSpread::JSXAttr(JSXAttr {
                span: DUMMY_SP,
                name: name.clone(),
                value: Some(value),
            });
    ...
    for attribute in attributes.iter_mut() {
        ...
        if !key.starts_with("use:") {
            ...
            {
                match visitor.evaluator.as_mut().unwrap().eval(expr) {
                ...
                || !matches!(expr.as_lit(), Some(Lit::Str(_)) | Some(Lit::Num(_)))
            {
                if key == "ref" {
                    ...
                    if !is_function && is_l_val(&*expr) {
                        let ref_ident = visitor.generate_uid_identifier("_ref$");
                        ...
                        results.exprs.insert(
                            ...
                            Expr::Cond(CondExpr {
                                ...
                                cons: Box::new(Expr::Call(CallExpr {
                                    span: DUMMY_SP,
                                    callee: Callee::Expr(Box::new(Expr::Ident(
                                        visitor.register_import_method("use"),
                                    ))),
                                    args: vec![
                                        ExprOrSpread {
                                            spread: None,
                    ...
                    if key.starts_with("on:") || key.starts_with("oncapture:") {
                        ...
                        results.exprs.push(Expr::Call(CallExpr {
                            ...
                            args: if key.starts_with("oncapture:") {
                                listener_options.push(ExprOrSpread {
                                    spread: None,
                    ...
                    } else if visitor.config.delegate_events
                        && (DELEGATED_EVENTS.contains(&ev.as_ref())
                            || visitor.config.delegated_events.contains(&ev.to_string()))
                    {
                        visitor.events.insert(ev.clone());
                        let el_ident = results.id.clone().unwrap();
                        let resolveable = detect_resolvable_event_handler(visitor, &*expr);
                        if let Expr::Array(ref arr_lit) = **expr {
                            ...
                            results.exprs.insert(
                                ...
                                Expr::Call(CallExpr {
                                    span: DUMMY_SP,
                                    callee: Callee::Expr(Box::new(Expr::Ident(
                                        visitor.register_import_method("addEventListener"),
                                    ))),
                                    args: vec![
                                        ExprOrSpread {
                                            spread: None,
                ...
                } else if !visitor.config.effect_wrapper.is_empty()
                    && (visitor.is_dynamic(&*expr, Some(*span), true, false, true, false)
                        || ((key == "classList" || key == "style")
                            && !(matches!(
                                visitor.evaluator.as_mut().unwrap().eval(&*expr),
                                Some(EvalResult::Lit(_))
                            ) || is_static_expr(&*expr))))
                {
                    ...
                    if key == "value" || key == "checked" {
                        results.post_exprs.push(Expr::Call(CallExpr {
                            ...
                            callee: Callee::Expr(Box::new(Expr::Ident(
                                visitor
                                    .register_import_method(&visitor.config.effect_wrapper.clone()),
                            ))),
                            args: vec![ExprOrSpread {
                                spread: None,
                                expr: Box::new(Expr::Arrow(ArrowExpr {
                    ...
                    if key == "textContent" {
                        next_elem = visitor.generate_uid_identifier("el$");
                    ...
                    results.exprs.push(visitor.set_attr_dom(
                        &elem.clone().unwrap(),
                        &key,
                        &*expr,
                        &AttrOptions {
...
fn context_to_custom_element<C: Comments>(
    visitor: &mut TransformVisitor<C>,
    results: &mut TemplateInstantiation,
) {
    results.exprs.push(Expr::Assign(AssignExpr {
        ...
        right: Box::new(Expr::Call(CallExpr {
            span: DUMMY_SP,
            callee: Callee::Expr(Box::new(Expr::Ident(
                visitor.register_import_method("getOwner"),
            ))),
            args: vec![],
            type_args: None,
        })),
...
fn process_spreads<C: Comments>(
    ...
    for attribute in &attributes {
        if let JSXAttrOrSpread::SpreadElement(el) = attribute {
            ...
            if visitor.is_dynamic(&el.expr, None, true, false, true, false) {
                dynamic_spread = true;
                if !match *el.expr {
            ...
            {
                dynamic = visitor.is_dynamic(expr, None, true, false, true, false);
            ...
            if flag {
                if dynamic {
                    ...
                    })) = attr.value
                    {
                        ...
                        {
                            let (_, b) = visitor.transform_condition(*ex.clone(), true, false);
                        ...
                        running_object.push(PropOrSpread::Prop(Box::new(Prop::Getter(
                            GetterProp {
                                span: DUMMY_SP,
                                key: prop,
                                type_ann: None,
                    ...
                    let value = if let Some(JSXAttrValue::JSXExprContainer(JSXExprContainer {
                        expr: JSXExpr::Expr(ref ex),
                        ..
                    })) = attr.value
                    {
    ...
    let props = if spread_args.len() == 1 && !dynamic_spread {
        ...
        let merge_props = visitor.register_import_method("mergeProps");
        Expr::Call(CallExpr {
            span: DUMMY_SP,
    ...
    let spread = visitor.register_import_method("spread");
    (
        filtered_attributes,
        Expr::Call(CallExpr {
            span: DUMMY_SP,
            ...
            args: vec![
                ...
                ExprOrSpread {
                    spread: None,
                    expr: Box::new(Expr::Lit(Lit::Bool(info.is_svg.into()))),
                },
                ExprOrSpread {
                    spread: None,
...
fn transform_children<C: Comments>(
    ...
    let last_element = find_last_element(visitor, &filtered_children);
    let child_nodes = filtered_children.iter().enumerate().fold(
        ...
        |mut memo, (index, child)| {
            ...
            let detect_expressions = detect_expressions(visitor, &filtered_children, index);
            let transformed = visitor.transform_node(
                child,
                &TransformInfo {
    ...
    child_nodes.iter().enumerate().for_each(|(index, child)| {
        ...
        if child.id.is_some() {
            ...
            let insert = visitor.register_import_method("insert");
            ...
            if wrapped_by_text(&child_nodes, index) {
                ...
                if let Some(placeholder) = next_placeholder.clone() {
                    ...
                    (expr_id, content_id) = create_placeholder(visitor, results, &temp_path, i, "");
                    i += 1;
                }
...
fn create_placeholder<C: Comments>(
    visitor: &mut TransformVisitor<C>,
    results: &mut TemplateInstantiation,
...
fn detect_expressions<C: Comments>(
    visitor: &mut TransformVisitor<C>,
    ...
    if index > 0 {
        ...
        ) && visitor.get_static_expression(node).is_none()
        {
            return true;
        }
    ...
    for child in children.iter().skip(index) {
        if let JSXElementChild::JSXExprContainer(JSXExprContainer { expr, .. }) = child {
            if !matches!(expr, JSXExpr::JSXEmptyExpr(_))
                && visitor.get_static_expression(child).is_none()
            {
                return true;
            }
            ...
            if visitor.config.context_to_custom_elements
            ...
            let next_children = e
                ...
                .filter(|c| filter_children(c))
                .collect::<Vec<&JSXElementChild>>();
            if !next_children.is_empty() && detect_expressions(visitor, &next_children, 0) {
                return true;
            }
        }
    }
...
fn find_last_element<C: Comments>(
    visitor: &mut TransformVisitor<C>,
    ...
    for i in (0i32..children.len() as i32).rev() {
        ...
        if matches!(child, JSXElementChild::JSXText(_))
            || visitor.get_static_expression(child).is_some()
        {
            last_element = i;
            break;
        }
